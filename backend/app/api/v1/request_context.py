...
def get_request_context(request: Request) -> RequestContext:
    tenant_source = request.headers.get("X-Tenant-ID") or request.session.get("tenant_id")
    user_source = request.headers.get("X-User-ID") or request.session.get("user_id")
    session_source = request.headers.get("X-Session-ID") or request.session.get("session_id")
